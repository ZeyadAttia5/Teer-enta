...
import {
    ...
    StarOutlined,
    HeartOutlined,
    HeartFilled, InfoCircleOutlined, MessageOutlined
...
Time: ${activity?.time}
Location: ${activity?.location ? `Latitude: ${activity?.location?.lat}, Longitude: ${activity?.location?.lng}` : "Location not specified"}
Price Range: ${currency?.code} ${activity?.price?.min ? (currency?.rate * activity?.price?.min).toFixed(2) : "N/A"} - ${activity?.price?.max ? (currency?.rate * activity?.price?.max).toFixed(2) : "N/A"}
Category: ${activity?.category?.category || "N/A"}

Special Discounts:
${activity?.specialDiscounts?.map(discount =>
            `${discount.discount}% OFF - ${discount.Description} (${discount.isAvailable ? "Available" : "Not Available"})`
                                ...
                                <div>
                                    ...
                                    <div className="flex flex-wrap gap-2">
                                        ...
                                        {activity.preferenceTags?.map((tag, index) => (
                                            <Tag key={index} color="blue" className="px-4 py-1 text-sm">
                                                {tag.tag}
                                            </Tag>
                                        ))}
                                    </div>
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-6 border-t border-gray-100 pt-6">
                                    <div className="flex items-center gap-3">
                                        ...
                                        <div>
                                            ...
                                            <Text className="text-gray-800 font-medium">{activity.createdBy?.username}</Text>
                                        </div>
                                    </div>
                                    <div className="flex items-center gap-3">
