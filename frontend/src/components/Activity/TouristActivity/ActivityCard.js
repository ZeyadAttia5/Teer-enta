...
import { CalendarOutlined, ClockCircleOutlined, FolderOutlined, DollarOutlined, EnvironmentOutlined } from "@ant-design/icons";
    ...
    <div className="flex justify-center items-center min-h-screen py-10">
      <div className="max-w-4xl w-full rounded-lg overflow-hidden shadow-lg bg-[#ffffff] text-third m-2">
        ...
        <div className="flex items-center p-4 bg-first text-[#ffffff] flex-col sm:flex-row">
          <h2 className="font-bold text-xl sm:text-5xl flex-grow break-words">{name}</h2>
            ...
            <div className="flex items-center space-x-2">
              <Tooltip title="Price" overlayClassName="bg-fourth">
                <p className="text-sm sm:text-base font-bold">
                  <DollarOutlined className="mr-0" /> {currencyCode} {price?.min ? (currencyRate * price.min).toFixed(1) : "N/A"}
                </p>
              </Tooltip>
              <Tooltip title="Price" overlayClassName="bg-fourth">
                <p className="text-sm sm:text-base font-bold">
                  {currencyCode} {price?.max ? (currencyRate * price.max).toFixed(1) : "N/A"}
                </p>
              </Tooltip>
            </div>
          </div>
