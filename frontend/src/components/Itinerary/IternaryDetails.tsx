...
const ItineraryDetails: React.FC = () => {
    ...
    - Price: ${currency?.code} ${(currency?.rate*itinerary?.price).toFixed(2)}
    ...
    return (
        <div className="bg-[#496989] min-h-screen flex items-center justify-center p-0 overflow-hidden w-full">
            <Space direction="vertical" size="large" className="relative w-full p-8">
                            ...
                            <Timeline>
                                {itinerary?.timeline.map((item, index) => (
                                    <Timeline.Item key={index}>
                                        ...
                                        {item.startTime && (
                                            <Text type="secondary" className="text-[#58A399]"> Starts
                                                at {item.startTime}</Text>
                                        )}
                        ...
                        >
                            <List
                                ...
                                renderItem={(date) => (
                                    <List.Item>
                                        <Card size="small"
                                              className="shadow-sm rounded-lg bg-white transition-transform duration-300 hover:bg-[#E2F4C5] hover:scale-105">
                                            <Space className="justify-between w-full">
                                                <Text
                                                    className="text-[#58A399]">{new Date(date.Date).toLocaleDateString()}</Text>
                ...
                {(itinerary?.comments.length ?? 0) > 0 && (
                    ...
                    >
                        <List
                            ...
                            renderItem={(comment) => (
                                <List.Item
                                    className="transition-transform duration-300 hover:bg-[#496989] hover:scale-105 hover:text-white">
                                    <List.Item.Meta
                ...
                {/* Footer Information */}
                <Card
                    className="text-center mt-4 bg-white shadow-lg rounded-lg transition-transform duration-300 hover:bg-[#E2F4C5] hover:scale-105 w-full">
                    <Space direction="vertical" size="small">
                        <Text type="secondary" className="text-[#58A399]">Created
                            by: {itinerary?.createdBy.username}</Text>
                        <Text type="secondary" className="text-[#58A399]">
