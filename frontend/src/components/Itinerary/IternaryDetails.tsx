...
const ItineraryDetails: React.FC = () => {
  ...
  const titleStyle =
    "text-lg font-semibold mb-4 flex items-center gap-2 text-white";
      ...
      <div className="min-h-screen p-8">
        ...
        >
          ...
          >
            ...
            <div className="space-y-4">
              ...
              <Row justify="center" className="text-center mb-0">
                <Title
                  level={2}
                  className="font-extrabold text-black-300 !text-black-300 mb-0 "
                  style={{ fontSize: "7rem", marginBottom: "0.15rem" }}
                >
                  {itinerary?.name}
                </Title>
              </Row>
                ...
                <List
                  ...
                  renderItem={(item) => (
                    <List.Item
                      ...
                      onClick={() =>
                        navigate(`../activityDetails/${item.activity?._id}`)
                      }
                    >
                      <Text className="text-black text-sm pl-3">
                        {item.activity?.name}
                      </Text>
                      <Tag className="bg-blue-500 text-white border-none">
                        {item.duration} min
                      </Tag>
                    </List.Item>
                  )}
                  ...
                  items={itinerary?.timeline.map((item, index) => ({
                    ...
                    children: (
                      <>
                        ...
                        {item.startTime && (
                          <Text className="text-black/80 block">
                            {" "}
                            Starts at {item.startTime}
                          </Text>
                        )}
                        {item.duration && (
                          <Tag className="bg-blue-500 text-white border-none mt-1">
                            {item.duration} min
                          </Tag>
                        )}
                      </>
          ...
          {(itinerary?.comments.length ?? 0) > 0 && (
            ...
            >
              <div className={titleStyle}>
                <UserOutlined className="text-[#4A90E2]" />{" "}
                {/* Icon color adjusted */}
          ...
          {(tourGuideComments?.length ?? 0) > 0 && (
            ...
            >
              <div className={titleStyle}>
                <UserOutlined className="text-[#4A90E2]" />{" "}
                {/* Icon color adjusted */}
                <span className="text-black">Tour Guide Reviews</span>{" "}
                {/* Title color changed to white */}
              </div>
          ...
          <Card
            className={`text-center ${cardStyle} ${gradientBg} rounded-lg shadow-lg mt-8 p-6`}
          >
            <div className="space-y-4">
