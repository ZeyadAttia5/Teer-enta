...
import { Card, Tag, Button, Space, Select, message, Segmented, Typography, Empty } from 'antd';
import {
    CalendarOutlined,
    DollarOutlined,
    UserOutlined,
...
const BookingGrid = () => {
        ...
        switch (timeFilter) {
            case 'upcoming':
                return items.filter(item => {
                    const itemDate = new Date(item.date || item.departureDate || item.checkInDate);
                    return itemDate > now;
                });
            case 'past':
                return items.filter(item => {
    ...
    const filteredItineraries = getFilteredItems(bookedItineraries, itineraryStatus);
    ...
    const filteredTransportations = getFilteredItems(bookedTransportations, transportationStatus);
    ...
    return (
        <div className="min-h-screen bg-gray-50 p-6 md:p-8">
            {/* Header Section */}
            <div className="mb-8">
                <Title level={2} className="mb-2">My Bookings</Title>
                <Text className="text-gray-500">Manage your travel bookings and reservations</Text>
            </div>
                        ...
                        <div className="flex flex-col sm:flex-row gap-4 w-full md:w-auto">
                            ...
                            <Select
                                    ...
                                    selectedType === 'Activities' ? activityStatus :
                                        selectedType === 'Itineraries' ? itineraryStatus :
                                            selectedType === 'Flights' ? flightStatus :
                                                selectedType === 'Hotels' ? hotelStatus :
                                ...
                                options={[
                                    ...
                                    { label: 'Pending', value: 'Pending', icon: <ClockCircleOutlined className="text-orange-500" /> },
                                    { label: 'Completed', value: 'Completed', icon: <CheckCircleOutlined className="text-green-500" /> },
                                    { label: 'Cancelled', value: 'Cancelled', icon: <CloseCircleOutlined className="text-red-500" /> }
                                ]}
                                optionRender={(option) => (
                                    <Space>
                                        {option.data.icon}
                    ...
                    <div className="flex flex-wrap gap-2">
                        {timeFilter !== 'all' && (
                            <Tag
                                ...
                                icon={timeFilter === 'upcoming' ? <CalendarOutlined /> : <HistoryOutlined />}
                            >
                            ...
                            <Tag
                                color={
                                    activityStatus === 'Pending' ? 'orange' :
                                        activityStatus === 'Completed' ? 'green' : 'red'
                                }
                                closable
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    {filteredActivities?.length > 0 ? (
                            ...
                            activity.activity && (
                                ...
                                >
                                    <div className="space-y-4">
                                        <div>
                                            <Title level={4} className="mb-2">{activity.activity.name}</Title>
                                        ...
                                        <div className="flex flex-col gap-2 pt-4">
                                            ...
                                            >
                                                {feedbackVisibility[activity._id]?.activity ? 'Hide Feedback' : 'Give Feedback'}
                                            </Button>
                                        </div>
                        ...
                        <div className="col-span-full">
                            <Empty
                                description={<span className="text-gray-500">No activities found</span>}
                                className="my-12"
                            />
                        </div>
                    )}
            ...
            {selectedType === 'Itineraries' && (
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    {filteredItineraries?.length > 0 ? (
                                ...
                                >
                                    <div className="space-y-4">
                                        <div className="flex justify-between items-start">
                                            <div>
                                                <Title level={4} className="mb-2">{itinerary.itinerary.name}</Title>
                                                <div className="flex items-center text-gray-500 text-sm">
                                                    <CalendarOutlined className="mr-2" />
                                        ...
                                        <div className="flex flex-col gap-2 pt-4">
                                            ...
                                            <Space direction="vertical" className="w-full">
                                                <Button
                                                    ...
                                                    icon={<UserOutlined />}
                                                >
                                                    {feedbackVisibility[itinerary._id]?.tourGuide ? 'Hide Guide Feedback' : 'Rate Tour Guide'}
                                                </Button>
                                                <Button
                                                    ...
                                                    icon={<CommentOutlined />}
                                                >
                                                    {feedbackVisibility[itinerary._id]?.itinerary ? 'Hide Feedback' : 'Rate Itinerary'}
                                                </Button>
                                            </Space>
                                        </div>

                                        {(feedbackVisibility[itinerary._id]?.tourGuide || feedbackVisibility[itinerary._id]?.itinerary) && (
                                            <div className="mt-4 border-t pt-4">
                        ...
                        <div className="col-span-full">
                            <Empty
                                description={<span className="text-gray-500">No itineraries found</span>}
                                className="my-12"
                            />
                        </div>
                    )}
            ...
            {selectedType === 'Flights' && (
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    {filteredFlights?.length > 0 ? (
                        filteredFlights.map((flight) => (
                            ...
                            >
                                <div className="space-y-4">
                                    <div className="flex justify-between items-start">
                                        <div>
                                            <Title level={4} className="mb-2">Flight Booking</Title>
                                            <div className="flex items-center text-gray-500 text-sm">
                                                <CalendarOutlined className="mr-2" />
                        ...
                        <div className="col-span-full">
                            <Empty
                                description={<span className="text-gray-500">No flights found</span>}
                                className="my-12"
                            />
                        </div>
                    )}
            ...
            {selectedType === 'Hotels' && (
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    {filteredHotels?.length > 0 ? (
                            ...
                            hotel.hotel && (
                                ...
                                >
                                    <div className="space-y-4">
                                        <div className="flex justify-between items-start">
                                            <div>
                                                <Title level={4} className="mb-2">{hotel.hotel.name}</Title>
                                                <div className="flex items-center text-gray-500 text-sm">
                                                    <CalendarOutlined className="mr-2" />
                                                    <span>
                        ...
                        <div className="col-span-full">
                            <Empty
                                description={<span className="text-gray-500">No hotels found</span>}
                                className="my-12"
                            />
                        </div>
                    )}
            ...
            {selectedType === 'Transportations' && (
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    {filteredTransportations?.length > 0 ? (
                            ...
                            transportation.transportation && (
                                ...
                                >
                                    <div className="space-y-4">
                                        <div className="flex justify-between items-start">
                                            <div>
                                                <Title level={4} className="mb-2">{transportation.transportation.name}</Title>
                                                <div className="flex items-center text-gray-500 text-sm">
                                                    <CalendarOutlined className="mr-2" />
                        ...
                        <div className="col-span-full">
                            <Empty
                                description={<span className="text-gray-500">No transportations found</span>}
                                className="my-12"
                            />
                        </div>
                    )}
