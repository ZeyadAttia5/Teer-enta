...
const TouristWelcome = ({ setFlag }) => {
  ...
  return (
      <div className="relative mt-16">
        <div className="relative flex flex-col items-center min-h-screen before:content-[''] before:bg-fit before:bg-no-repeat before:size-full before:absolute before:z-[0] before:animate-tourist-background">
          {/* Hero Section */}
          <Fade
              className="text-white absolute left-4 sm:left-[100px] top-[15%] w-[90%] sm:w-auto"
              direction="up"
              cascade
          >
            <h1 className="text-3xl sm:text-[50px] lg:text-[70px] font-semibold leading-tight sm:leading-[50px] lg:leading-[100px]">
          ...
          <Fade
              className="text-white absolute left-4 sm:left-[100px] top-[40%] sm:top-[44%]"
              direction="up"
              cascade
          >
        <span className="flex gap-2">
          <img
              src={logo}
              alt="Teer-Enta Logo"
              className="w-[30px] sm:w-[40px] h-auto rounded-full"
          />
          ...
          <Fade
              className="text-white absolute left-4 sm:left-[100px] top-[47%] sm:top-[51%] text-lg sm:text-2xl font-bold w-[90%] sm:w-auto"
              direction="up"
              cascade
          >
          ...
          {/* Navigation Buttons */}
          <div className="absolute top-[65%] sm:top-[72%] w-full px-4 sm:w-3/4 flex flex-col sm:flex-row gap-4 sm:gap-0 sm:justify-around">
            <button
                className={`px-4 sm:px-12 py-3 sm:py-4 flex items-center justify-center rounded-lg gap-2 font-bold focus:outline-none active:outline-none ring-0 text-lg sm:text-2xl ${
                    selectedButton === 1
            ...
            <button
                className={`px-4 sm:px-12 py-3 sm:py-4 font-bold rounded-lg flex items-center justify-center gap-2 text-lg sm:text-2xl ${
                    selectedButton === 2
                        ? "bg-backgroundColor text-first"
                        : "bg-transparent text-white"
                }`}
            ...
            <button
                className={`px-4 sm:px-12 py-3 sm:py-4 flex gap-2 items-center justify-center rounded-lg font-bold ring-0 text-lg sm:text-2xl ${
                    selectedButton === 3
                        ? "bg-backgroundColor text-first"
                        : "bg-transparent text-white"
                }`}
        ...
        <span className="text-2xl sm:text-4xl font-bold text-first ml-0 sm:ml-12 block mt-8">
          Top historical places around the world
        </span>

              <div className="flex justify-center">
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mt-4 w-full sm:w-[90%]">
                  {historicalPlaces?.slice(0, 4).map((place, index) => (
                      <Link key={index} to={`/historicalPlace/details/${place?._id}`}>
                        <div className="relative hover:shadow-xl hover:border-b-2 transition-transform border-b-2 border-backgroundColor ease-in-out duration-300 hover:border-black">
                          ...
                          />
                          <span
                              className="absolute top-1 left-1 rounded-xl font-bold text-lg sm:text-xl text-white p-2"
                ...
                {historicalPlaces?.length > 4 && (
                    <div className="flex flex-col justify-center mt-4">
                      <Button type="danger" className="w-fit rounded-full" href="/historicalPlace">
                        <svg
                            xmlns="http://www.w3.org/2000/svg"
                            fill="none"
                            viewBox="0 0 24 24"
              ...
              {itineraries?.length > 0 && (
                  <span className="text-2xl sm:text-4xl font-bold text-first ml-0 sm:ml-12 mt-16 block mb-3">
            Top itineraries you can't miss
          </span>
              )}

              <div className="flex justify-center">
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 w-full sm:w-[90%] px-4 sm:px-0">
                ...
                {itineraries?.length > 3 && (
                    <div className="flex flex-col justify-center">
                      <Button type="danger" className="w-fit rounded-full" href="/itinerary">
                        <svg
                            xmlns="http://www.w3.org/2000/svg"
                            fill="none"
                            viewBox="0 0 24 24"
              ...
              {touristActivities?.length > 0 && (
                  <span className="text-2xl sm:text-4xl font-bold mb-3 text-first ml-0 sm:ml-12 mt-8 block">
            Top activities you can't miss
              ...
              <div className="flex justify-center mb-8">
                ...
                {touristActivities?.length > 3 && (
                    <div className="flex flex-col justify-center mt-4">
                      <Button type="danger" className="w-fit rounded-full" href="/touristActivities">
                        <svg
                            xmlns="http://www.w3.org/2000/svg"
                            fill="none"
                            viewBox="0 0 24 24"
