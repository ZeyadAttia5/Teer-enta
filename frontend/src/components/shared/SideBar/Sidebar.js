...
const Sidebar = ({visible, onClose}) => {
    ...
    >
        ...
        >
            ...
            {user && (user.userRole === "Admin" || user.userRole === "TourismGovernor") && (
                <Menu.SubMenu key="sub5" title="Tags" icon={<TagsOutlined/>}>
                    {renderMenuItems([user && user.userRole === "Admin" && (<Menu.Item
                        key="1"
                        onClick={() => handleClick("/preference-tags")}
                        icon={<TagsOutlined/>}
                    >
                        Preference Tags
                    </Menu.Item>), user && (user.userRole === "Admin" || user.userRole === "TourismGovernor") && (
                        <Menu.Item key="2" onClick={() => handleClick("/tags")} icon={<TagsOutlined/>}>
                            Tags
                        </Menu.Item>),])}
                </Menu.SubMenu>)}
            {(user === null || (user && (user.userRole === "TourismGovernor" || user.userRole === "Tourist" || user.userRole === "Admin"))) && renderMenuItems([
                <Menu.Item
                    key="3"
                ...
                >
                    Historical Places
                </Menu.Item>, user && user.userRole === "TourismGovernor" && (<Menu.Item
                    key="12"
            ...
            {(user === null || (user && (user.userRole === "TourGuide" || user.userRole === "Tourist" || user.userRole === "Admin"))) &&
                <Menu.SubMenu key="sub77" title="Itrenaries" icon={<MdOutlineLocalActivity/>}>
                    {renderMenuItems([<Menu.Item
                        key="4"
                        onClick={() => handleClick(user && (user.userRole === "TourGuide" || user.userRole === "Admin") ? "/itinerary/tourguide_itineraries" : "/itinerary")}
                        icon={<CarryOutOutlined/>}>
                        Itinerary
                    </Menu.Item>, user && user.userRole === "TourGuide" && (<Menu.Item
                        key="55"
                        onClick={() => handleClick("/itinerary/unActiveIternaries")}
                        icon={<CarryOutOutlined/>}
                    >
                        UnActive Itinerary
                    </Menu.Item>), user && user.userRole === "Admin" && (<Menu.Item
                        key="15"
                        onClick={() => handleClick("/itinerary/flaggedIternaries")}
                        icon={<FlagOutlined/>}
                    >
                        Flagged Itinerary
                    </Menu.Item>), user && user.userRole === "TourGuide" && (
            ...
            {user && (user.userRole === "Advertiser" || user.userRole === "Admin" || user.userRole === "Tourist") && (
                <Menu.SubMenu key="sub3" title="Activities" icon={<MdOutlineLocalActivity/>}>
                    ...
                    >
                        Activity Categories
                    </Menu.Item>), user && user.userRole !== "Tourist" && (<Menu.Item
                        key="7"
                        onClick={() => handleClick("/activities")}
                        icon={<MdOutlineLocalActivity/>}
                    >
                        Activities
                    </Menu.Item>), user && user.userRole === "Advertiser" && (<Menu.Item
                        key="1000000"
            ...
            {user && user.userRole === "Advertiser" && (<Menu.Item
                key="19"
            ...
            {(user === null || (user && (user.userRole === "Seller" || user.userRole === "Tourist" || user.userRole === "Admin"))) && (
                <Menu.SubMenu key="sub6" title="Products" icon={<ShoppingOutlined/>}>
                    {renderMenuItems([<Menu.Item key="13" onClick={() => handleClick("/products")}
                                                 icon={<ShoppingOutlined/>}>
                        All Products
                    </Menu.Item>, user && user.userRole === "Tourist" && (<Menu.Item
                        key="15"
                    ...
                    >
                        Orders History
                    </Menu.Item>), user && (user.userRole === "Seller" || user.userRole === "Admin") && (<>
                        <Menu.Item
                            key="14"
            ...
            {user && user.userRole === "Admin" && (<Menu.SubMenu key="sub2" title="Users" icon={<UserOutlined/>}>
                {renderMenuItems([<Menu.Item key="9" onClick={() => handleClick("/allUsers")}
                                             icon={<UserOutlined/>}>
                    Show Users
                </Menu.Item>, <Menu.Item key="10" onClick={() => handleClick("/addUser")} icon={<UserOutlined/>}>
                    Add User
                </Menu.Item>, <Menu.Item key="11" onClick={() => handleClick("/pendingUsers")} icon={<UserOutlined/>}>
                    Pending Users
                </Menu.Item>, <Menu.Item key="34" onClick={() => handleClick("/AllDeletionRequests")}
                                         icon={<UserOutlined/>}>
            ...
            {user && (user.userRole === "Tourist" || user.userRole === "Admin") && renderMenuItems([user && user.userRole === "Admin" && (
                <Menu.Item
                    key="17"
            ...
            {user && user.userRole === "Admin" && (<Menu.SubMenu key="promo" title="Promo codes" icon={<GiftOutlined/>}>
                {renderMenuItems([<Menu.Item
                    key="1701"
